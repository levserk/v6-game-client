...
define('modules/game_manager',['EE', 'instances/room', 'instances/turn', 'instances/game_event', 'instances/time'],
    ...
    GameManager.prototype.checkConnectionDelay = function(time){
        ...
        if (delay > 12000 && !!$('.dialogNotice').length && ((this.state == 'sending_turn') || !time.user.isPlayer )) {
            this.client.viewsManager.dialogsView.showNotice(this.client.locale['dialogs']['badConnection'], 2000);
        }

        if (time.user.isPlayer && this.state == 'sending_turn' && delay > 15000){
            this.client.doReconnect();
            clearInterval(this.timeInterval);
            console.warn('game_manager;', 'checkConnectionDelay', 'reconnect!', time);
            return;
        }

        if(!time.user.isPlayer && delay > 25000) {
            this.client.socket.ws.close();
            clearInterval(this.timeInterval);
            console.warn('game_manager;', 'checkConnectionDelay', 'reconnect!', time);
        }
...
define('views/dialogs',['underscore', 'text!tpls/v6-dialogRoundResult.ejs'], function(_, tplRoundResultStr) {
    ...
    var dialogs = (function() {
        ...
        function userLeave(user) {
            hideNotification();
            var html = user.isPlayer ? locale['playerLeave'] : locale['user'] + ' <b>' + user.userName + '</b> ' + locale['opponentLeave'];
        ...
        function hideGameMessages(notHideRoundResult) {
            $('.' + GAME_CLASS).dialog("close");
            if (!notHideRoundResult){
        ...
        var ChatView = Backbone.View.extend({
            ...
            clickDialogAction: function(e) {
                ...
                switch (actionObj.action){
                    ...
                    case 'dialog': this.client.chatManager.openDialog(actionObj.userId, actionObj.userName); break;
            ...
            showMenu: function(e, userId, userName) {
                ...
                setTimeout(function() {
                    ...
                    // hide/show dialog
                    if (userId == this.client.getPlayer().userId || userId == '0' || this.client.gameManager.inGame()){
                        this.$menu.find('li[data-action=dialog]').hide();
                    } else {
                        this.$menu.find('li[data-action=dialog]').show();
                    }
            ...
            _closeDialog: function(target){
                ...
                this.tabs['private'] = false;
                this.$el.find('.tabs div[data-type="' + 'room' + '"]').hide();
                this.tabs['room'] = false;
            },
...
function(GameManager, InviteManager, UserList, Socket, ViewsManager, ChatManager, HistoryManager, RatingManager,
    ...
    Client.prototype.doReconnect = function() {
        if (!this.opts.autoReconnect) return;
        this.socket.ws.close();
        this.socket.clear();
        this.socket.emit('disconnection');
        this.viewsManager.dialogsView.showNotice(this.locale['dialogs']['reConnection'],3000);
    };
